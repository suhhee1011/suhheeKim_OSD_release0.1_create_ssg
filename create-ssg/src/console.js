...
var cliApplication = function (args) {
    ...
    if (options.version) {
        ...
        let jsonData, jsonOutput, filename = "";
        ...
        if (options.config) {
            ...
            if (Object.entries(jsExist).length === 0) {
                console.log("Sorry, Json file is empty. We are using all default values")
            } else {
        ...
        if (result == filename) {
            const str = "";
            if (filename == "") { filename = "Sherlock-Holmes-Selected-Stories"; }
