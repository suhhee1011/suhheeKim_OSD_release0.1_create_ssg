...
function createHTML(filename, TextArr, url, jsonOutput) {
    ...
    for (let filenum = 0; filenum < TextArr.length; filenum++) {
        ...
        fs.readFile(TextArr.length > 1 ? './Texts/'.concat(filename, "/", TextArr[filenum][0], ".", TextArr[filenum][1]) : './Texts/'.concat(TextArr[filenum][0], ".", TextArr[filenum][1]), 'utf8', function (error, data) {
           ...
           `
            ...
            //check if output value exists in the json file
           ...
           if(jsonOutput != ""){
            ...
            fs.writeFile(outputPath.concat(TextArr[filenum][0], ".html"), html, function (err) {
    ...
    export function cli(args) {
        ...
        //check if json file is empty when option is config.
        if(Object.entries(jsExist).length === 0){
            console.log("Sorry, Json file is empty.")
        }
        else{
            if (options.version) {
                ...
                if (result == filename) {
                    ...
                    fs.readdir('./Texts/'.concat(filename), function (error, filelist) {
                        ...
                        createHTML(filename, TextArr, url,jsonOutput);
                    });
